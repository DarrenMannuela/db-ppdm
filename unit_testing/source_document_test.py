...
class TestField(unittest.TestCase):
    ...
    def test_select_source_document(self):
        primary_key = 'A1'
        cursor = connection.cursor()
        query = "SELECT * FROM SOURCE_DOCUMENT WHERE SOURCE_DOCUMENT_ID = 'A1'"
    ...
    def test_alter_source_document(self):
        try:
            cursor = connection.cursor()
            query = "UPDATE  SOURCE_DOCUMENT SET SOURCE_DOCUMENT_ID = 'B1' WHERE SOURCE_DOCUMENT_ID = 'A1'"
            cursor.execute(query)
            connection.commit()
    ...
    def test_delete_source_document(self):
        try:
            cursor = connection.cursor()
            query = "DELETE FROM SOURCE_DOCUMENT WHERE SOURCE_DOCUMENT_ID = 'A1'"
            cursor.execute(query)
            connection.commit()
            #self.assertEqual(primary_key, get_key, "Data doesnt exist")
        except oracledb.Error as e:
