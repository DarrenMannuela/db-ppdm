...
func SetAfe(c *fiber.Ctx) error {
	...
	afe.Data_type = "3D Seismic Field Data Stored In Media"
...
func DeleteAfe(c *fiber.Ctx) error {
	...
	var t3d_seismic_field_data_stored_in_media_ids []string

	for rows.Next() {
		var t3d_seismic_field_data_stored_in_media_id string
		if err := rows.Scan(&t3d_seismic_field_data_stored_in_media_id); err != nil {
			return err
		}

		t3d_seismic_field_data_stored_in_media_ids = append(t3d_seismic_field_data_stored_in_media_ids, t3d_seismic_field_data_stored_in_media_id)
	...
	for i := 0; i < len(t3d_seismic_field_data_stored_in_media_ids); i++ {
		_, err = tx.Exec(`DELETE FROM t3d_seismic_field_data_stored_in_media_table WHERE id = :1`, t3d_seismic_field_data_stored_in_media_ids[i])
		if err != nil {
			tx.Rollback()
