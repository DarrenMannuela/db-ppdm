...
func GetAfe(c *fiber.Ctx) error {
	rows, err := db.Query("SELECT * FROM well_samples_table_afe")
	if err != nil {
...
func GetAfeByNum(c *fiber.Ctx) error {
	...
	rows, err := db.Query("SELECT * FROM well_samples_table_afe WHERE afe_number = :1", afeNumber)
	if err != nil {
		return c.Status(500).SendString(err.Error())
	}
	defer rows.Close()
...
func DeleteAfe(c *fiber.Ctx) error {
	...
	rows, err := tx.Query(`SELECT well_samples_id FROM well_samples_table_workspace WHERE afe_number = :1`, afeNumber)
	if err != nil {
		tx.Rollback()
		fmt.Println("WELL_SAMPLES_TABLE_WORKSPACE")
		return err
	}

	var well_samples_ids []string

	for rows.Next() {
		var well_samples_id string
		if err := rows.Scan(&well_samples_id); err != nil {
			return err
		}

		well_samples_ids = append(well_samples_ids, well_samples_id)

	}

	_, err = tx.Exec(`DELETE FROM well_samples_table_workspace WHERE afe_number = :1`, afeNumber)
	if err != nil {
		tx.Rollback()
		fmt.Println("WELL_SAMPLES_TABLE_WORKSPACE")
		return err
	...
	for i := 0; i < len(well_samples_ids); i++ {
		_, err = tx.Exec(`DELETE FROM well_samples_table WHERE id = :1`, well_samples_ids[i])
		if err != nil {
			tx.Rollback()
	...
	delRows, err := tx.Exec(`DELETE FROM well_samples_table_afe WHERE afe_number = :1`, afeNumber)
	if err != nil {
		tx.Rollback()
		fmt.Println("WELL_SAMPLES_TABLE_AFE")
		return err
...
func PatchWorkspaceAfe(c *fiber.Ctx) error {
	...
	if afe.Workspace_name != nil {
		...
		_, err = tx.Exec(`UPDATE well_samples_table_afe SET workspace_name = :1 WHERE afe_number = :2`, afe.Workspace_name, afeNumber)
	...
	if afe.Kkks_name != nil {
		...
		_, err = tx.Exec(`UPDATE well_samples_table_afe SET kkks_name = :1 WHERE afe_number = :2`, afe.Kkks_name, afeNumber)
	...
	if afe.Working_area != nil {
		...
		_, err = tx.Exec(`UPDATE well_samples_table_afe SET working_area = :1 WHERE afe_number = :2`, afe.Working_area, afeNumber)
	...
	if afe.Submission_type != nil {
		...
		_, err = tx.Exec(`UPDATE well_samples_table_afe SET submission_type = :1 WHERE afe_number = :2`, afe.Submission_type, afeNumber)
