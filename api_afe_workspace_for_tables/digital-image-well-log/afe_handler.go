...
func SetAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`INSERT INTO digital_image_well_log_table_afe (afe_number, workspace_name, kkks_name, working_area, submission_type, data_type) VALUES (:1, :2, :3, :4, :5, :6)`, afe.Afe_number, afe.Workspace_name, afe.Kkks_name, afe.Working_area, afe.Submission_type, afe.Data_type)
	if err != nil {
		tx.Rollback()
...
func PutAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`UPDATE digital_image_well_log_table_afe SET workspace_name = :1, kkks_name = :2, working_area = :3, submission_type = :4, data_type = :5 WHERE afe_number = :6`,
