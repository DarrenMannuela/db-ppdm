...
func SetAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`INSERT INTO digital_well_log_table_afe (afe_number, workspace_name, kkks_name, working_area, submission_type, data_type, Email) VALUES (:1, :2, :3, :4, :5, :6, :7)`, afe.Afe_number, afe.Workspace_name, afe.Kkks_name, afe.Working_area, afe.Submission_type, afe.Data_type, afe.Email)
	if err != nil {
		tx.Rollback()
...
func PutAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`UPDATE digital_well_log_table_afe SET workspace_name = :1, kkks_name = :2, working_area = :3, submission_type = :4, data_type = :5, Email = :6 WHERE afe_number = :7`,
...
func DeleteAfe(c *fiber.Ctx) error {
	...
	var digital_well_log_ids []string

	for rows.Next() {
		var digital_well_log_id string
		if err := rows.Scan(&digital_well_log_id); err != nil {
			return err
		}

		digital_well_log_ids = append(digital_well_log_ids, digital_well_log_id)
	...
	for i := 0; i < len(digital_well_log_ids); i++ {
		_, err = tx.Exec(`DELETE FROM digital_well_log_table WHERE id = :1`, digital_well_log_ids[i])
		if err != nil {
			tx.Rollback()
...
func PatchWorkspaceAfe(c *fiber.Ctx) error {
	...
	if afe.Email != "" {
		...
		_, err = tx.Exec(`UPDATE digital_well_log_table_afe SET Email = :1 WHERE afe_number = :2`, afe.Email, afeNumber)
