...
func GetWorkspace(c *fiber.Ctx) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&workspace.Id, &workspace.Afe_number, &workspace.Digital_well_log_id); err != nil {
...
func GetWorkspaceByAfe(c *fiber.Ctx) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&curRow.Id, &curRow.Afe_number, &curRow.Digital_well_log_id); err != nil {
			return err // Exit if we get an error
		}
...
func SetWorkspace(c *fiber.Ctx) error {
	...
	var digital_well_log_id_exist string
	err = tx.QueryRow("SELECT id FROM digital_well_log_table where id = :1", workspace.Digital_well_log_id).Scan(&digital_well_log_id_exist)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`INSERT INTO digital_well_log_table_workspace (afe_number, digital_well_log_id) VALUES (:2, :3)`, workspace.Afe_number, workspace.Digital_well_log_id)
	if err != nil {
		tx.Rollback()
...
func PutWorkspace(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`UPDATE digital_well_log_table_workspace SET afe_number = :1, digital_well_log_id = :2 WHERE id = :3`,
		workspace.Afe_number, workspace.Digital_well_log_id, id)
	if err != nil {
		tx.Rollback()
		fmt.Println("DIGITAL_WELL_LOG_TABLE_WORKSPACE")
		return err
...
func DeleteWorkspace(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`DELETE FROM digital_well_log_table_workspace WHERE digital_well_log_id = :1`, id)
	if err != nil {
...
func PatchWorkspace(c *fiber.Ctx) error {
	...
	if workspace.Digital_well_log_id != 0 {

		_, err = tx.Exec(`UPDATE digital_well_log_table_workspace SET digital_well_log_id = :1 WHERE id = :2`,
			workspace.Digital_well_log_id, id)
		if err != nil {
			tx.Rollback()
