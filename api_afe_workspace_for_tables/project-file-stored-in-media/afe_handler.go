...
func SetAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`INSERT INTO project_file_stored_in_media_table_afe (afe_number, workspace_name, kkks_name, working_area, submission_type, data_type, Email) VALUES (:1, :2, :3, :4, :5, :6, :7)`, afe.Afe_number, afe.Workspace_name, afe.Kkks_name, afe.Working_area, afe.Submission_type, afe.Data_type, afe.Email)
	if err != nil {
		tx.Rollback()
...
func PutAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`UPDATE project_file_stored_in_media_table_afe SET workspace_name = :1, kkks_name = :2, working_area = :3, submission_type = :4, data_type = :5, Email = :6 WHERE afe_number = :7`,
...
func PatchWorkspaceAfe(c *fiber.Ctx) error {
	...
	if afe.Email != "" {
		...
		_, err = tx.Exec(`UPDATE project_file_stored_in_media_table_afe SET Email = :1 WHERE afe_number = :2`, afe.Email, afeNumber)
