...
func PatchDigitalTechnicalReport(c *fiber.Ctx) error{
    ...
    if idExist != ""{
        if dtrt.Ba_long_name != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET ba_long_name = :1 WHERE id = :2`, dtrt.Ba_long_name, id)
        ...
        if dtrt.Ba_type != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET ba_type = :1 WHERE id = :2`, dtrt.Ba_type, id)
        ...
        if dtrt.Area_id != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET area_id = :1 WHERE id = :2`, dtrt.Area_id, id)
        ...
        if dtrt.Area_type != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET area_type = :1 WHERE id = :2`, dtrt.Area_type, id)
        ...
        if dtrt.Title != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET title = :1 WHERE id = :2`, dtrt.Title, id)
        ...
        if dtrt.Creator_name != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET creator_name = :1 WHERE id = :2`, dtrt.Creator_name, id)
        ...
        if dtrt.Create_date != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET create_date = :1 WHERE id = :2`, dtrt.Create_date, id)
        ...
        if dtrt.Page_count != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET page_count = :1 WHERE id = :2`, dtrt.Page_count, id)
        ...
        if dtrt.Media_type != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET media_type = :1 WHERE id = :2`, dtrt.Media_type, id)
        ...
        if dtrt.Digital_format != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET digital_format = :1 WHERE id = :2`, dtrt.Digital_format, id)
        ...
        if dtrt.Document_type != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET document_type = :1 WHERE id = :2`, dtrt.Document_type, id)
        ...
        if dtrt.Item_category != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET item_category = :1 WHERE id = :2`, dtrt.Item_category, id)
        ...
        if dtrt.Item_sub_category != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET item_sub_category = :1 WHERE id = :2`, dtrt.Item_sub_category, id)
        ...
        if dtrt.Ba_long_name_2 != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET ba_long_name_2 = :1 WHERE id = :2`, dtrt.Ba_long_name_2, id)
        ...
        if dtrt.Ba_type_2 != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET ba_type_2 = :1 WHERE id = :2`, dtrt.Ba_type_2, id)
        ...
        if dtrt.Data_store_name != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET data_store_name = :1 WHERE id = :2`, dtrt.Data_store_name, id)
        ...
        if dtrt.Original_file_name != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET original_file_name = :1 WHERE id = :2`, dtrt.Original_file_name, id)
        ...
        if dtrt.Password != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET password = :1 WHERE id = :2`, dtrt.Password, id)
        ...
        if dtrt.Digital_size != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET digital_size = :1 WHERE id = :2`, dtrt.Digital_size, id)
        ...
        if dtrt.Digital_size_uom != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET digital_size_uom = :1 WHERE id = :2`, dtrt.Digital_size_uom, id)
        ...
        if dtrt.Remark != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET remark = :1 WHERE id = :2`, dtrt.Remark, id)
        ...
        if dtrt.Source != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET source = :1 WHERE id = :2`, dtrt.Source, id)
        ...
        if dtrt.Qc_status != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET qc_status = :1 WHERE id = :2`, dtrt.Qc_status, id)
        ...
        if dtrt.Checked_by_ba_id != nil{
             _, err = tx.Exec(`UPDATE digital_technical_report_table SET checked_by_ba_id = :1 WHERE id = :2`, dtrt.Checked_by_ba_id, id)
