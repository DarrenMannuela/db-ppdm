...
func PatchT2DSeismicNavigationDataStoredInMedia(c *fiber.Ctx) error{
    ...
    if idExist != ""{
        if tsndsimt.Ba_long_name != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET ba_long_name = :1 WHERE id = :2`, tsndsimt.Ba_long_name, id)
        ...
        if tsndsimt.Ba_type != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET ba_type = :1 WHERE id = :2`, tsndsimt.Ba_type, id)
        ...
        if tsndsimt.Area_id != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET area_id = :1 WHERE id = :2`, tsndsimt.Area_id, id)
        ...
        if tsndsimt.Area_type != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET area_type = :1 WHERE id = :2`, tsndsimt.Area_type, id)
        ...
        if tsndsimt.Acqtn_survey_name != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET acqtn_survey_name = :1 WHERE id = :2`, tsndsimt.Acqtn_survey_name, id)
        ...
        if tsndsimt.Seis_dimension != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET seis_dimension = :1 WHERE id = :2`, tsndsimt.Seis_dimension, id)
        ...
        if tsndsimt.Process_date != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET process_date = :1 WHERE id = :2`, tsndsimt.Process_date, id)
        ...
        if tsndsimt.Shot_by != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET shot_by = :1 WHERE id = :2`, tsndsimt.Shot_by, id)
        ...
        if tsndsimt.Line_name != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET line_name = :1 WHERE id = :2`, tsndsimt.Line_name, id)
        ...
        if tsndsimt.Digital_format != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET digital_format = :1 WHERE id = :2`, tsndsimt.Digital_format, id)
        ...
        if tsndsimt.Media_type != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET media_type = :1 WHERE id = :2`, tsndsimt.Media_type, id)
        ...
        if tsndsimt.Tape_number != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET tape_number = :1 WHERE id = :2`, tsndsimt.Tape_number, id)
        ...
        if tsndsimt.Ba_long_name_2 != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET ba_long_name_2 = :1 WHERE id = :2`, tsndsimt.Ba_long_name_2, id)
        ...
        if tsndsimt.Ba_type_2 != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET ba_type_2 = :1 WHERE id = :2`, tsndsimt.Ba_type_2, id)
        ...
        if tsndsimt.Data_store_name != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET data_store_name = :1 WHERE id = :2`, tsndsimt.Data_store_name, id)
        ...
        if tsndsimt.Data_store_type != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET data_store_type = :1 WHERE id = :2`, tsndsimt.Data_store_type, id)
        ...
        if tsndsimt.Location_id != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET location_id = :1 WHERE id = :2`, tsndsimt.Location_id, id)
        ...
        if tsndsimt.Remark != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET remark = :1 WHERE id = :2`, tsndsimt.Remark, id)
        ...
        if tsndsimt.Source != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET source = :1 WHERE id = :2`, tsndsimt.Source, id)
        ...
        if tsndsimt.Qc_status != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET qc_status = :1 WHERE id = :2`, tsndsimt.Qc_status, id)
        ...
        if tsndsimt.Checked_by_ba_id != nil{
             _, err = tx.Exec(`UPDATE t2d_seismic_navigation_data_stored_in_media_table SET checked_by_ba_id = :1 WHERE id = :2`, tsndsimt.Checked_by_ba_id, id)
