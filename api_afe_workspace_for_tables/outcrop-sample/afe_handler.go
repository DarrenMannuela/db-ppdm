...
func GetAfe(c *fiber.Ctx) error {
	rows, err := db.Query("SELECT * FROM outcrop_sample_table_afe")
	if err != nil {
...
func GetAfeByNum(c *fiber.Ctx) error {
	...
	rows, err := db.Query("SELECT * FROM outcrop_sample_table_afe WHERE afe_number = :1", afeNumber)
	if err != nil {
		return c.Status(500).SendString(err.Error())
	}
	defer rows.Close()
...
func SetAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`INSERT INTO outcrop_sample_table_afe (afe_number, workspace_name, kkks_name, working_area, submission_type, data_type, Email) VALUES (:1, :2, :3, :4, :5, :6, :7)`, afe.Afe_number, afe.Workspace_name, afe.Kkks_name, afe.Working_area, afe.Submission_type, afe.Data_type, afe.Email)
	if err != nil {
		tx.Rollback()
		fmt.Println("OUTCROP_SAMPLE_TABLE_AFE")
		return err
	}
...
func PutAfe(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET workspace_name = :1, kkks_name = :2, working_area = :3, submission_type = :4, data_type = :5, Email = :6 WHERE afe_number = :7`,
...
func DeleteAfe(c *fiber.Ctx) error {
	...
	rows, err := tx.Query(`SELECT outcrop_sample_id FROM outcrop_sample_table_workspace WHERE afe_number = :1`, afeNumber)
	if err != nil {
		tx.Rollback()
		fmt.Println("OUTCROP_SAMPLE_TABLE_WORKSPACE")
		return err
	}
	...
	_, err = tx.Exec(`DELETE FROM outcrop_sample_table_workspace WHERE afe_number = :1`, afeNumber)
	if err != nil {
		tx.Rollback()
		fmt.Println("OUTCROP_SAMPLE_TABLE_WORKSPACE")
		return err
	...
	delRows, err := tx.Exec(`DELETE FROM outcrop_sample_table_afe WHERE afe_number = :1`, afeNumber)
	if err != nil {
		tx.Rollback()
		fmt.Println("OUTCROP_SAMPLE_TABLE_AFE")
		return err
...
func PatchWorkspaceAfe(c *fiber.Ctx) error {
	...
	if afe.Workspace_name != nil {
		...
		_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET workspace_name = :1 WHERE afe_number = :2`, afe.Workspace_name, afeNumber)
	...
	if afe.Kkks_name != nil {
		...
		_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET kkks_name = :1 WHERE afe_number = :2`, afe.Kkks_name, afeNumber)
	...
	if afe.Working_area != nil {
		...
		_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET working_area = :1 WHERE afe_number = :2`, afe.Working_area, afeNumber)
	...
	if afe.Submission_type != nil {
		...
		_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET submission_type = :1 WHERE afe_number = :2`, afe.Submission_type, afeNumber)
	...
	if afe.Data_type != "" {
		...
		_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET data_type = :1 WHERE afe_number = :2`, afe.Data_type, afeNumber)
	...
	if afe.Email != "" {
		...
		_, err = tx.Exec(`UPDATE outcrop_sample_table_afe SET Email = :1 WHERE afe_number = :2`, afe.Email, afeNumber)
