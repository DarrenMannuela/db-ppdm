...
func GetWorkspace(c *fiber.Ctx) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&workspace.Id, &workspace.Afe_number, &workspace.2d_seismic_navigation_digital_data_id); err != nil {
...
func GetWorkspaceByAfe(c *fiber.Ctx) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&curRow.Id, &curRow.Afe_number, &curRow.2d_seismic_navigation_digital_data_id); err != nil {
			return err // Exit if we get an error
		}
...
func SetWorkspace(c *fiber.Ctx) error {
	...
	var 2d_seismic_navigation_digital_data_id_exist string
	err = tx.QueryRow("SELECT id FROM 2d_seismic_navigation_digital_data_table where id = :1", workspace.2d_seismic_navigation_digital_data_id).Scan(&2d_seismic_navigation_digital_data_id_exist)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`INSERT INTO 2d_seismic_navigation_digital_data_table_workspace (afe_number, 2d_seismic_navigation_digital_data_id) VALUES (:2, :3)`, workspace.Afe_number, workspace.2d_seismic_navigation_digital_data_id)
	if err != nil {
		tx.Rollback()
...
func PutWorkspace(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`UPDATE 2d_seismic_navigation_digital_data_table_workspace SET afe_number = :1, 2d_seismic_navigation_digital_data_id = :2 WHERE id = :3`,
		workspace.Afe_number, workspace.2d_seismic_navigation_digital_data_id, id)
	if err != nil {
		tx.Rollback()
		fmt.Println("2D_SEISMIC_NAVIGATION_DIGITAL_DATA_TABLE_WORKSPACE")
		return err
...
func DeleteWorkspace(c *fiber.Ctx) error {
	...
	_, err = tx.Exec(`DELETE FROM 2d_seismic_navigation_digital_data_table_workspace WHERE 2d_seismic_navigation_digital_data_id = :1`, id)
	if err != nil {
...
func PatchWorkspace(c *fiber.Ctx) error {
	...
	if workspace.2d_seismic_navigation_digital_data_id != 0 {

		_, err = tx.Exec(`UPDATE 2d_seismic_navigation_digital_data_table_workspace SET 2d_seismic_navigation_digital_data_id = :1 WHERE id = :2`,
			workspace.2d_seismic_navigation_digital_data_id, id)
		if err != nil {
			tx.Rollback()
