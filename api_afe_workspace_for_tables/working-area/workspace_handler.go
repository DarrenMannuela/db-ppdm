...
func GetWorkspace(c *fiber.Ctx) error {
	rows, err := db.Query("SELECT * FROM working_area_table_workspace")
	if err != nil {
...
func GetWorkspaceByAfe(c *fiber.Ctx) error {
	...
	rows, err := tx.Query("SELECT * FROM working_area_table_workspace WHERE afe_number = :1", id)
	if err != nil {
		return err
	}
...
func PatchWorkspace(c *fiber.Ctx) error {
	...
	if workspace.Afe_number != nil {
		...
		_, err = tx.Exec(`UPDATE working_area_table_workspace SET afe_number = :1 WHERE id = :2`,
