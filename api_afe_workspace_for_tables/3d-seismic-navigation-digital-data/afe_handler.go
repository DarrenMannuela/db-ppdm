...
func DeleteAfe(c *fiber.Ctx) error {
	...
	var 3d_seismic_navigation_digital_data_ids []string

	for rows.Next() {
		var 3d_seismic_navigation_digital_data_id string
		if err := rows.Scan(&3d_seismic_navigation_digital_data_id); err != nil {
			return err
		}

		3d_seismic_navigation_digital_data_ids = append(3d_seismic_navigation_digital_data_ids, 3d_seismic_navigation_digital_data_id)
	...
	for i := 0; i < len(3d_seismic_navigation_digital_data_ids); i++ {
		_, err = tx.Exec(`DELETE FROM t3d_seismic_navigation_digital_data_table WHERE id = :1`, 3d_seismic_navigation_digital_data_ids[i])
		if err != nil {
			tx.Rollback()
